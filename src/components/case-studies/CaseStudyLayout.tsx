...
export default function MultiVendorMarketplacePage() {
  ...
  // Marketplace themed animation - multiple sellers, products, transactions
  useEffect(() => {
    ...
    // Vendor store icons (representing different sellers)
    const vendors: { x: number; y: number; icon: string; pulse: number; opacity: number }[] = []
    const vendorIcons = ['🛒', '👕', '📱', '💻', '👟', '👜', '⌚', '🎧']
    for (let i = 0; i < 12; i++) {
      vendors.push({
    ...
    const products: { x: number; y: number; price: string; size: number; speedY: number; opacity: number }[] = []
    const prices = ['$49', '$129', '$19', '$299', '$79', '$999', '$249', '$59']
    for (let i = 0; i < 20; i++) {
      products.push({
        x: Math.random() * canvas.width,
        y: Math.random() * canvas.height,
    ...
    // Transaction flows (buyer to seller)
    const transactions: { from: number; to: number; progress: number; speed: number }[] = []
    for (let i = 0; i < 18; i++) {
      transactions.push({
        from: Math.floor(Math.random() * vendors.length),
        to: Math.floor(Math.random() * vendors.length),
    ...
    // Rating stars
    const stars: { x: number; y: number; rating: number; size: number; opacity: number }[] = []
    for (let i = 0; i < 15; i++) {
      stars.push({
        x: Math.random() * canvas.width,
        y: Math.random() * canvas.height,
    ...
    const animate = () => {
      ...
      transactions.forEach((tx) => {
        ...
        if (tx.progress > 1) tx.progress = 0

        const fromVendor = vendors[tx.from]
        const toVendor = vendors[tx.to]

        if (fromVendor && toVendor) {
          // Draw transaction line
          ctx.beginPath()
          ctx.moveTo(fromVendor.x, fromVendor.y)
          ctx.lineTo(toVendor.x, toVendor.y)
          ctx.strokeStyle = 'rgba(255, 46, 159, 0.15)'
          ctx.lineWidth = 1.2
          ctx.stroke()

          // Draw money packet
          const currentX = fromVendor.x + (toVendor.x - fromVendor.x) * tx.progress
          const currentY = fromVendor.y + (toVendor.y - fromVendor.y) * tx.progress
      ...
      vendors.forEach((vendor) => {
        vendor.pulse += 0.02
        const yOffset = Math.sin(vendor.pulse) * 5

        ctx.font = '28px Arial'
        ctx.fillStyle = `rgba(91, 108, 255, ${vendor.opacity})`
        ctx.fillText(vendor.icon, vendor.x, vendor.y + yOffset)

        // Vendor badge
        ctx.beginPath()
        ctx.arc(vendor.x + 15, vendor.y - 15, 4, 0, Math.PI * 2)
      ...
      products.forEach((product) => {
        product.y -= product.speedY
        if (product.y < 0) {
          product.y = canvas.height
          product.x = Math.random() * canvas.width
        }

        ctx.font = `${product.size}px monospace`
        ctx.fillStyle = `rgba(255, 46, 159, ${product.opacity})`
        ctx.fillText(product.price, product.x, product.y)
      })
  ...
  const processSteps = [
    {
      ...
      description: 'Analyzed seller onboarding and buyer journey',
      icon: '🔍',
    ...
    },
    {
      ...
      description: 'Built scalable multi-tenant marketplace platform',
      icon: '🏗️',
    ...
    },
    {
      ...
      description: 'Implemented automated commission splitting',
      icon: '💰',
      gradient: 'from-[#FF2E9F] to-[#5B6CFF]',
    },
    {
      ...
      description: 'Onboarded 5,000+ sellers in 3 months',
      icon: '🚀',
      gradient: 'from-[#5B6CFF] to-[#FF2E9F]',
    },
  ]
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            <nav className="flex justify-center items-center gap-2 text-sm text-gray-500">
              ...
              <span>•</span>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF2E9F] to-[#5B6CFF]">Multi-vendor Marketplace</span>
            </nav>
          </motion.div>
          ...
          >
            <span className="text-white">Multi-vendor</span>{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF2E9F] to-[#5B6CFF]">Marketplace</span>
          </motion.h1>
          ...
          >
            <div className="backdrop-blur-sm bg-white/5 rounded-full border border-white/10 px-6 py-3 hover:bg-white/10 transition-all">
              <span className="text-sm font-medium gradient-text">5,000+ Sellers</span>
            </div>
            <div className="backdrop-blur-sm bg-white/5 rounded-full border border-white/10 px-6 py-3 hover:bg-white/10 transition-all">
              <span className="text-sm font-medium gradient-text">1M+ Products</span>
            </div>
            <div className="backdrop-blur-sm bg-white/5 rounded-full border border-white/10 px-6 py-3 hover:bg-white/10 transition-all">
              <span className="text-sm font-medium gradient-text">500K Orders/mo</span>
            </div>
          </motion.div>
        </div>
      </section>
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              >
                ...
                <p className="text-lg text-gray-400 leading-relaxed mb-8">
                  Gulf Mart wanted to launch a marketplace platform to compete with regional players,
                  but needed a solution that could handle thousands of sellers, millions of products,
                  and complex commission structures.
                </p>
                
                <div className="space-y-4">
                  {[
                    'Manual seller onboarding taking weeks',
                    'Complex commission splitting for multiple sellers',
                    'No unified inventory management',
                    'Disparate payment systems for sellers',
                  ].map((item, index) => (
          ...
          <div className="backdrop-blur-lg bg-white/5 rounded-3xl border border-white/10 p-12 shadow-2xl">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              >
                ...
                <p className="text-lg text-gray-400 leading-relaxed mb-8">
                  We built a scalable multi-vendor marketplace platform with automated seller onboarding,
                  real-time commission splitting, and powerful search capabilities.
                </p>

                <div className="space-y-4">
                  {[
                    'Automated seller onboarding with KYC verification',
                    'Smart commission engine for complex seller payouts',
                    'Real-time inventory sync across all sellers',
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-xl text-gray-400 max-w-3xl mx-auto">
              How we launched the marketplace with 5,000+ sellers in 6 months
            </p>
          </motion.div>
      ...
      <section className="relative py-24 bg-midnight border-t border-white/5">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="backdrop-blur-lg bg-white/5 rounded-3xl border border-white/10 p-12 shadow-2xl">
            ...
            >
              <h2 className="text-4xl md:text-5xl font-bold mb-4" style={{ fontFamily: 'var(--font-clash)' }}>
                Marketplace <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF2E9F] to-[#5B6CFF]">Architecture</span>
              </h2>
            </motion.div>

            <div className="grid md:grid-cols-4 gap-6 text-center">
              {[
                { icon: '🛍️', title: 'Buyer Frontend', tech: 'Next.js' },
                { icon: '🏪', title: 'Seller Dashboard', tech: 'React' },
                { icon: '💰', title: 'Payment Engine', tech: 'Stripe Connect' },
                { icon: '📊', title: 'Search & Catalog', tech: 'Elasticsearch' },
              ].map((item, index) => (
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="backdrop-blur-lg bg-white/5 rounded-3xl border border-white/10 p-12 shadow-2xl">
            ...
            >
              ...
              <p className="text-xl text-gray-400 max-w-3xl mx-auto">
                The technologies powering Gulf Mart's marketplace platform
              </p>
            </motion.div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="backdrop-blur-lg bg-white/5 rounded-3xl border border-white/10 p-12 shadow-2xl">
            ...
            >
              ...
              <p className="text-xl text-gray-400 max-w-3xl mx-auto">
                Measurable results from marketplace launch
              </p>
            </motion.div>
            ...
            >
              <div className="inline-block backdrop-blur-lg bg-white/5 px-8 py-4 rounded-full border border-white/10">
                <p className="text-lg font-medium">
                  <span className="gradient-text">AED 250M GMV in Year 1</span>
                  <span className="text-gray-400 mx-2">•</span>
                  <span className="gradient-text">40% repeat seller rate</span>
                  <span className="text-gray-400 mx-2">•</span>
                  <span className="gradient-text">4.8★ seller satisfaction</span>
                </p>
              </div>
            </motion.div>
          </div>
          ...
          >
            ...
            <div className="relative backdrop-blur-lg bg-white/5 rounded-3xl border border-white/10 group-hover:border-white/20 p-12 text-center transition-all">
              ...
              <p className="text-xl text-gray-300 italic mb-8 leading-relaxed">
                "The marketplace platform has exceeded all our expectations. We onboarded 5,000 sellers in
                just 3 months, and our GMV hit AED 250M in the first year. The automated commission splitting
                and seller dashboard have made it incredibly easy for vendors to manage their businesses.
                This platform is now the backbone of our e-commerce strategy."
              </p>
              <div>
                <p className="font-bold text-white text-lg">Faisal Al Qassimi</p>
                <p className="text-sm text-gray-500">Director of E-commerce, Gulf Mart</p>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="backdrop-blur-lg bg-white/5 rounded-3xl border border-white/10 p-12 shadow-2xl">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {[
                { title: 'Omnichannel Retail Platform', client: 'Gulf Retail Group', slug: 'omnichannel-gulf-retail' },
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p className="text-xl text-gray-400 mb-10 max-w-2xl mx-auto">
              Let's discuss how our multi-vendor platform can help you scale your marketplace business.
            </p>
            <Link href="/contact">
