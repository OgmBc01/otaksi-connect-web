...
const caseStudies = [
  ...
  },
  {
    ...
    client: 'Dubai Police',
    description: 'AI-powered traffic management system reducing congestion by 25%.',
    image: '/images/solutions/government/case-study-3.jpg',
    gradient: 'linear-gradient(135deg, #FF2E9F 0%, #5B6CFF 100%)',
  ...
  },
  {
    ...
    description: 'Secure document management system for 10,000+ government employees.',
    image: '/images/solutions/government/case-study-4.jpg',
    gradient: 'linear-gradient(135deg, #5B6CFF 0%, #FF2E9F 100%)',
...
export default function CaseStudies() {
  ...
  // Unique animated background for Case Studies section (government metrics)
  useEffect(() => {
    ...
    const metrics: { label: string; value: number; target: number; x: number; y: number; speed: number }[] = [
      { label: 'Citizen Satisfaction', value: 85, target: 95, x: 200, y: 200, speed: 0.1 },
      { label: 'Service Efficiency', value: 78, target: 90, x: 500, y: 250, speed: 0.15 },
      { label: 'Digital Adoption', value: 72, target: 85, x: 800, y: 300, speed: 0.2 },
    ]

    // Create floating citizen feedback
    const feedback: { x: number; y: number; rating: number; size: number; speedY: number; opacity: number }[] = []
    ...
    const animate = () => {
      ...
      // Draw floating feedback
      feedback.forEach((fb) => {
        fb.y -= fb.speedY

        if (fb.y < 0) {
          fb.y = canvas.height
          fb.x = Math.random() * canvas.width
        }

        ctx.font = `${fb.size}px monospace`
        ctx.fillStyle = `rgba(91, 108, 255, ${fb.opacity})`
        ctx.fillText('★'.repeat(fb.rating), fb.x, fb.y)
      })
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xl text-gray-400 max-w-3xl mx-auto">
            See how we've helped government entities transform citizen services and 
        ...
        >
          ...
          >
            <span>View All Government Case Studies</span>
