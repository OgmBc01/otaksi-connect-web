...
export default function CTASection() {
  ...
  // Unique animated background for CTA section (patient recovery/healing)
  useEffect(() => {
    ...
    // Create rising health indicators
    const indicators: { x: number; y: number; value: number; target: number; label: string; speed: number }[] = [
      { x: 200, y: 200, value: 65, target: 95, label: 'Patient Satisfaction', speed: 0.1 },
      { x: 500, y: 250, value: 70, target: 98, label: 'Recovery Rate', speed: 0.15 },
      { x: 800, y: 300, value: 55, target: 90, label: 'Efficiency', speed: 0.2 },
    ]

    // Create floating hearts (care symbols)
    const hearts: { x: number; y: number; size: number; speedY: number; opacity: number }[] = []
    ...
    const animate = () => {
      ...
      // Update and draw health indicators
      indicators.forEach((indicator) => {
        if (indicator.value < indicator.target) {
          indicator.value += indicator.speed
        }

        // Draw progress circle
        ctx.beginPath()
        ctx.arc(indicator.x, indicator.y, 40, 0, Math.PI * 2)
        ctx.strokeStyle = 'rgba(91, 108, 255, 0.2)'
        ctx.lineWidth = 4
        ctx.stroke()

        // Draw progress fill
        const angle = (indicator.value / 100) * Math.PI * 2
        ctx.beginPath()
        ctx.arc(indicator.x, indicator.y, 40, 0, angle)
        ...
        ctx.fillText(indicator.label, indicator.x - 50, indicator.y - 50)
        ctx.fillText(`${Math.round(indicator.value)}%`, indicator.x - 15, indicator.y + 60)
      })
      ...
      hearts.forEach((heart) => {
        ...
        ctx.fillStyle = `rgba(255, 46, 159, ${heart.opacity})`
      ...
      for (let x = 0; x < canvas.width; x += 20) {
        const y = canvas.height - 50 - Math.sin(x * 0.01 + time) * 30 - 20
        ctx.lineTo(x, y)
      }
  ...
  return (
    <section className="relative py-24 bg-midnight overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <Image
          src="/images/solutions/healthcare/cta-bg.jpg"
          alt="Healthcare CTA"
          fill
      ...
      <div className="absolute bottom-20 right-20 opacity-20 hidden lg:block">
        <div className="text-6xl animate-float animation-delay-2000">❤️</div>
      </div>
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
              <Link href="mailto:healthcare@otaksi.ae" className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors group">
                ...
                <span>healthcare@otaksi.ae</span>
              </Link>
          ...
          >
            <p className="text-xs text-gray-600">
              ⚡ Free consultation | DHA compliant | HL7/FHIR experts | 24/7 support
            </p>
          </motion.div>
        </motion.div>
      </div>
