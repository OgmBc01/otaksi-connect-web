...
foreach ($template in $templates.templates) {
    ...
    $techList = ($template.technologies | ForEach-Object { "              <span className='px-4 py-2 glass-card rounded-full text-sm text-gray-300 border border-white/10'>$_</span>" }) -join "`n"
    
    # Generate metrics list as a string
    $metricItems = ($template.metrics | ForEach-Object { 
@"
...
"@
    ...
    Write-Host "✅ Created $($template.id) with full sections and animation placeholders" -ForegroundColor Green
}

Write-Host "`n🎉 SUCCESS! All 15 case study templates created!" -ForegroundColor Green
Write-Host "`n📁 Location: app\case-studies\" -ForegroundColor Cyan
Write-Host "`n🔧 NEXT STEPS:" -ForegroundColor Yellow
Write-Host "1. Run the first two scripts to update references" -ForegroundColor Yellow
Write-Host "2. Review each case study in app\case-studies\" -ForegroundColor Yellow
Write-Host "3. We'll work together to create UNIQUE ANIMATIONS for each case study" -ForegroundColor Yellow
Write-Host "4. Replace placeholder images with actual screenshots/diagrams" -ForegroundColor Yellow
Write-Host "5. Customize content to tell each unique story" -ForegroundColor Yellow